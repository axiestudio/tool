 ...
 * tools/turso-docs.js
 *
 * Fetch Turso database documentation — no API key required.
 * Turso exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Turso doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Turso docs
 */

const BASE = "https://docs.turso.tech";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Turso documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Turso doc page as clean Markdown. " +
      "Use a path like '/cli' or '/sdk/ts'.",
    parameters: {
      path: "string — doc path, e.g. '/sdk/ts'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Turso documentation for a keyword or topic. " +
      "Good for questions like 'embedded replicas' or 'libSQL'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/sdk/ts" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown turso-docs tool: "${toolName}"`);
  }
}

export function normalizeTursoDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Turso llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeTursoDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Turso page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch Turso full docs: ${res.status}`);
  }

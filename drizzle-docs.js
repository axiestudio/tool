/**
 * tools/drizzle-docs.js
 *
 * Fetch Drizzle ORM documentation — no API key required.
 * Drizzle exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Drizzle doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Drizzle docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Drizzle ORM documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Drizzle doc page as clean Markdown. " +
      "Examples: '/docs/select', '/docs/migrations', '/docs/drizzle-kit-generate'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/select'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Drizzle documentation for a keyword or topic. " +
      "Good for questions like 'relations', 'migrations', 'rls', 'neon serverless'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/relations" });
 * const { result }   = await invoke("search-docs", { query: "rls row level security" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown drizzle-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Drizzle doc path:
 *  - Strips absolute URLs to path-only
 *  - Removes .md / index.md suffixes
 *  - Ensures leading /
 *  - Strips query params and hash
 */
export function normalizeDrizzleDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching Drizzle index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Drizzle docs index not found (404). The site may have changed its structure."
        : `Failed to fetch Drizzle llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeDrizzleDocPath(path);
  ...
  throw new Error(`Drizzle page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading Drizzle docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Drizzle full docs archive not found (404). The site may have changed its structure."
        : `Could not load Drizzle full docs: ${res.status}`,
    );
  }

/**
 * tools/turborepo-docs.js
 *
 * Fetch Turborepo monorepo build system documentation — no API key required.
 * Turborepo exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Turborepo doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Turborepo docs
 */

const BASE = "https://turborepo.dev";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Turborepo documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Turborepo doc page as clean Markdown. " +
      "Use a path like '/getting-started' or '/reference/configuration'.",
    parameters: {
      path: "string — doc path, e.g. '/crafting-your-repository/caching'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Turborepo documentation for a keyword or topic. " +
      "Good for questions like 'how does caching work' or 'turbo.json configuration'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/getting-started" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown turborepo-docs tool: "${toolName}"`);
  }
}

export function normalizeTurborepoDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Turborepo llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeTurborepoDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Turborepo page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch Turborepo full docs: ${res.status}`);
  }

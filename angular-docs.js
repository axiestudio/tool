/**
 * tools/angular-docs.js
 *
 * Fetch Angular web application framework documentation — no API key required.
 * Angular exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Angular doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Angular docs
 */

const BASE = "https://angular.dev";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Angular documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Angular doc page as clean Markdown. " +
      "Use a path like '/guide/components' or '/guide/signals'.",
    parameters: {
      path: "string — doc path, e.g. '/guide/di'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Angular documentation for a keyword or topic. " +
      "Good for questions like 'how do signals work' or 'angular routing'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/guide/components" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown angular-docs tool: "${toolName}"`);
  }
}

export function normalizeAngularDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Angular llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeAngularDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Angular page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch Angular full docs: ${res.status}`);
  }

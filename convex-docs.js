/**
 * tools/convex-docs.js
 *
 * Fetch Convex reactive backend platform documentation — no API key required.
 * Convex exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Convex doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Convex docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Convex documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Convex doc page as clean Markdown. " +
      "Use a path like '/quickstart' or '/functions'.",
    parameters: {
      path: "string — doc path, e.g. '/database' or '/auth'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Convex documentation for a keyword or topic. " +
      "Good for questions like 'how do mutations work' or 'database schema'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/quickstart" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown convex-docs tool: "${toolName}"`);
  }
}

export function normalizeConvexDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Convex llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeConvexDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Convex page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch Convex full docs: ${res.status}`);
  }

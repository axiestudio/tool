 ...
 * tools/vue-docs.js
 *
 * Fetch Vue.js framework documentation — no API key required.
 * Vue.js exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Vue.js doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Vue.js docs
 */

const BASE = "https://vuejs.org";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Vue.js documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Vue.js doc page as clean Markdown. " +
      "Use a path like '/guide/introduction' or '/api/composition-api-setup'.",
    parameters: {
      path: "string — doc path, e.g. '/guide/essentials/reactivity-fundamentals'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Vue.js documentation for a keyword or topic. " +
      "Good for questions like 'how does reactivity work' or 'composition api'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/guide/introduction" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown vue-docs tool: "${toolName}"`);
  }
}

export function normalizeVueDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Vue.js llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeVueDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Vue.js page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch Vue.js docs: ${res.status}`);
  }

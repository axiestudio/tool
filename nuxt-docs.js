 ...
 * tools/nuxt-docs.js
 *
 * Fetch Nuxt (Vue.js meta-framework) documentation — no API key required.
 * Nuxt exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Nuxt doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Nuxt docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Nuxt documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Nuxt doc page as clean Markdown. " +
      "Use a path like '/docs/getting-started/introduction' or '/docs/api/composables/use-fetch'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/guide/concepts/auto-imports'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Nuxt documentation for a keyword or topic. " +
      "Good for questions like 'how do auto imports work' or 'nuxt server routes'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/getting-started/introduction" });
 * const { result }   = await invoke("search-docs", { query: "auto imports" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown nuxt-docs tool: "${toolName}"`);
  }
}

export function normalizeNuxtDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Nuxt llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeNuxtDocPath(path);

  // Try Accept: text/markdown first
  ...
  // Fall back to raw URL pattern (nuxt.com/raw/docs/...)
  const rawRes = await fetch(`${BASE}/raw${cleanPath}.md`);

  if (!rawRes.ok) {
    throw new Error(`Nuxt page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  // Try full docs first, fall back to llms.txt
  let res = await fetch(`${BASE}/llms-full.txt`);
  if (!res.ok) {
    res = await fetch(`${BASE}/llms.txt`);
    if (!res.ok) throw new Error(`Failed to fetch Nuxt docs: ${res.status}`);
  }

 ...
 * tools/vite-docs.js
 *
 * Fetch Vite build tool documentation — no API key required.
 * Vite exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Vite doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Vite docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Vite documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Vite doc page as clean Markdown. " +
      "Examples: '/guide/', '/config/', '/guide/env-and-mode'.",
    parameters: {
      path: "string — doc path, e.g. '/guide/features'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Vite documentation for a keyword or topic. " +
      "Good for questions like 'HMR', 'env variables', 'library mode', 'SSR'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/guide/features" });
 * const { result }   = await invoke("search-docs", { query: "HMR hot module replacement" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown vite-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Vite doc path:
 *  - Strips the base URL prefix
 *  - Ensures leading /
 *  - Removes /index.md suffix
 *  - Removes .md extension
 */
export function normalizeViteDocPath(raw) {
  if (!raw) return "";
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Vite llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeViteDocPath(path);
  const res = await fetch(`${BASE}${cleanPath}`, {
    headers: { Accept: "text/markdown" },
  });

  if (!res.ok) {
    throw new Error(`Vite page not found: ${path} (${res.status}). Use get-index to discover available pages.`);
  }

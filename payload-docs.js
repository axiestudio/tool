/**
 * tools/payload-docs.js
 *
 * Fetch Payload CMS documentation — no API key required.
 * Payload exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Payload doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Payload docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Payload CMS documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Payload CMS doc page as clean Markdown. " +
      "Examples: '/docs/getting-started/what-is-payload', '/docs/fields/overview', '/docs/access-control/overview'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/configuration/overview'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Payload CMS documentation for a keyword or topic. " +
      "Good for questions about collections, globals, fields, hooks, access control, authentication.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/fields/overview" });
 * const { result }   = await invoke("search-docs", { query: "access control hooks" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown payload-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Payload doc path:
 *  - Strips the https://payloadcms.com prefix
 *  - Removes /index.md and .md suffixes
 *  - Ensures leading /
 */
export function normalizePayloadDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching Payload index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Payload docs index not found (404). The site may have changed its structure."
        : `Failed to fetch Payload llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizePayloadDocPath(path);
  ...
  throw new Error(`Payload page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading Payload docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Payload full docs archive not found (404). The site may have changed its structure."
        : `Could not load Payload full docs: ${res.status}`,
    );
  }
...
function scoreChunk(chunk, lowerQuery, queryTerms) {
  ...
  if (firstLine.startsWith("#") && firstLine.toLowerCase().includes(lowerQuery)) {
    score += 5;
  }
  ...
  for (const term of queryTerms) {
    ...
    if (matches.length > 0) {
      ...
      if (firstLine.startsWith("#") && firstLine.toLowerCase().includes(term)) {
        score += 3;
      }
    }
  }

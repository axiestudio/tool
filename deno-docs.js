 ...
 * tools/deno-docs.js
 *
 * Fetch Deno documentation — no API key required.
 * Deno exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Deno doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Deno docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Deno documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Deno doc page as clean Markdown. " +
      "Examples: '/runtime/', '/runtime/fundamentals/typescript/', '/deploy/', '/examples/'.",
    parameters: {
      path: "string — doc path, e.g. '/runtime/fundamentals/typescript/'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Deno documentation for a keyword or topic. " +
      "Good for questions like 'permissions', 'FFI', 'deploy', 'KV database'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/runtime/fundamentals/typescript/" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown deno-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Deno doc path:
 *  - Strips absolute URLs to path-only
 *  - Removes .md / index.md suffixes
 *  - Ensures leading /
 *  - Ensures trailing /
 */
export function normalizeDenoDocPath(path) {
  if (!path) return "/";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching Deno index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Deno docs index not found (404). The site may have changed its structure."
        : `Failed to fetch Deno llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeDenoDocPath(path);
  ...
  throw new Error(`Deno page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading Deno docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Deno full docs archive not found (404). The site may have changed its structure."
        : `Could not load Deno full docs: ${res.status}`,
    );
  }
...
function getPageCandidates(cleanPath) {
  ...
  if (cleanPath.endsWith("/")) {
    const bare = cleanPath.slice(0, -1);
    candidates.push(`${BASE}${bare}.md`);

 ...
 * tools/hono-docs.js
 *
 * Fetch Hono web framework documentation — no API key required.
 * Hono exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Hono doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Hono docs
 */

const BASE = "https://hono.dev";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Hono documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Hono doc page as clean Markdown. " +
      "Use a path like '/docs/getting-started' or '/docs/api/hono'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/guides/middleware'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Hono documentation for a keyword or topic. " +
      "Good for questions like 'how does routing work' or 'hono middleware'.",
    parameters: {
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown hono-docs tool: "${toolName}"`);
  }
}

export function normalizeHonoDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Hono llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeHonoDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Hono page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Hono full docs: ${res.status}`);

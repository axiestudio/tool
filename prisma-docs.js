/**
 * tools/prisma-docs.js
 *
 * Fetch Prisma ORM documentation — no API key required.
 * Prisma exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Prisma doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Prisma docs
 */

const BASE = "https://www.prisma.io";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Prisma documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Prisma doc page as clean Markdown. " +
      "Use a path like '/docs/getting-started' or '/docs/concepts/components/prisma-client'.",
    parameters: {
  ...
  },
  {
    name: "search-docs",
    description:
      "Searches all Prisma documentation for a keyword or topic. " +
      "Good for questions like 'how do migrations work' or 'prisma relations'.",
    parameters: {
 ...
 * const { result }   = await invoke("search-docs", { query: "migrations" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown prisma-docs tool: "${toolName}"`);
  }
}

export function normalizePrismaDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Prisma llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizePrismaDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Prisma page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Prisma full docs: ${res.status}`);

 ...
 * tools/tauri-docs.js
 *
 * Fetch Tauri desktop app framework documentation — no API key required.
 * Tauri exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Tauri doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Tauri docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Tauri v2 documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Tauri doc page as clean Markdown. " +
      "Examples: '/start/', '/develop/', '/plugin/barcode-scanner'.",
    parameters: {
      path: "string — doc path, e.g. '/start/create-project/'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Tauri documentation for a keyword or topic. " +
      "Good for questions like 'IPC', 'window management', 'plugin system', 'updater'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/start/create-project/" });
 * const { result }   = await invoke("search-docs", { query: "IPC commands" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown tauri-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Tauri doc path:
 *  - Strips the base URL prefix
 *  - Ensures leading /
 *  - Removes /index.md suffix
 *  - Removes .md extension
 */
export function normalizeTauriDocPath(raw) {
  if (!raw) return "";
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Tauri llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeTauriDocPath(path);
  const res = await fetch(`${BASE}${cleanPath}`, {
    headers: { Accept: "text/markdown" },
  });

  if (!res.ok) {
    throw new Error(`Tauri page not found: ${path} (${res.status}). Use get-index to discover available pages.`);
  }

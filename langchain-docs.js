/**
 * tools/langchain-docs.js
 *
 * Fetch LangChain JS documentation — no API key required.
 * LangChain exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available LangChain JS doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all LangChain JS docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all LangChain JS documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific LangChain JS doc page as clean Markdown. " +
      "Examples: '/docs/introduction', '/docs/concepts/agents', '/docs/how_to/tool_calling'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/tutorials/rag'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all LangChain JS documentation for a keyword or topic. " +
      "Good for questions like 'tool calling', 'agents', 'retrieval', 'streaming'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/concepts/agents" });
 * const { result }   = await invoke("search-docs", { query: "tool calling" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown langchain-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a LangChain doc path:
 *  - Strips absolute URL prefix
 *  - Removes .md / index.md suffixes
 *  - Ensures leading /
 */
export function normalizeLangchainDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching LangChain index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "LangChain docs index not found (404). The site may have changed its structure."
        : `Failed to fetch LangChain llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeLangchainDocPath(path);
  ...
  throw new Error(`LangChain page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading LangChain docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "LangChain full docs archive not found (404). The site may have changed its structure."
        : `Could not load LangChain full docs: ${res.status}`,
    );
  }
...
function scoreChunk(chunk, lowerQuery, queryTerms) {
  ...
  if (firstLine.startsWith("#")) {
    const lowerHeading = firstLine.toLowerCase();
    for (const term of queryTerms) {

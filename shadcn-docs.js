/**
 * tools/shadcn-docs.js
 *
 * Fetch shadcn/ui documentation — no API key required.
 * shadcn/ui exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available shadcn/ui doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all shadcn/ui docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all shadcn/ui documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific shadcn/ui doc page as clean Markdown. " +
      "Examples: '/docs/installation', '/docs/components/button', '/docs/components/dialog'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/components/button'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all shadcn/ui documentation for a keyword or topic. " +
      "Good for questions like 'data table', 'dark mode', 'theming', 'form validation'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/components/button" });
 * const { result }   = await invoke("search-docs", { query: "data table" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown shadcn-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a shadcn/ui doc path:
 *  - Strips absolute URLs to path-only
 *  - Removes .md / index.md suffixes
 *  - Ensures leading /
 *  - Strips query params and hash
 */
export function normalizeShadcnDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching shadcn/ui index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "shadcn/ui docs index not found (404). The site may have changed its structure."
        : `Failed to fetch shadcn/ui llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeShadcnDocPath(path);
  ...
  throw new Error(`shadcn/ui page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading shadcn/ui docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "shadcn/ui full docs archive not found (404). The site may have changed its structure."
        : `Could not load shadcn/ui full docs: ${res.status}`,
    );
  }

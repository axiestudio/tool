/**
 * tools/fastify-docs.js
 *
 * Fetch Fastify web framework documentation — no API key required.
 * Fastify exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Fastify doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Fastify docs
 */

const BASE = "https://fastify.dev";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Fastify documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Fastify doc page as clean Markdown. " +
      "Use a path like '/docs/getting-started' or '/docs/guides/plugins'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/guides/plugins'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Fastify documentation for a keyword or topic. " +
      "Good for questions like 'how does routing work' or 'fastify plugins'.",
    parameters: {
 ...
 * const { result }   = await invoke("search-docs", { query: "plugins" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown fastify-docs tool: "${toolName}"`);
  }
}

export function normalizeFastifyDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Fastify llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeFastifyDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Fastify page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Fastify full docs: ${res.status}`);

/**
 * tools/xstate-docs.js
 *
 * Fetch XState documentation — no API key required.
 * XState: state machines and statecharts for the modern web.
 * Docs are exposed as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available XState doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all XState docs
 */

const BASE = "https://stately.ai";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all XState documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific XState doc page as clean Markdown. " +
      "Use a path like '/docs/xstate' or '/docs/machines'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/guards'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all XState documentation for a keyword or topic. " +
      "Good for questions like 'how do state machines work' or 'xstate actors'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/xstate" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown xstate-docs tool: "${toolName}"`);
  }
}

export function normalizeXstateDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch XState llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeXstateDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`XState page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch XState full docs: ${res.status}`);

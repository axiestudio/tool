/**
 * tools/storybook-docs.js
 *
 * Fetch Storybook UI workshop documentation — no API key required.
 * Storybook exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Storybook doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Storybook docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Storybook documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Storybook doc page as clean Markdown. " +
      "Examples: '/docs/writing-stories', '/docs/addons', '/docs/configure'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/writing-stories'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Storybook documentation for a keyword or topic. " +
      "Good for questions like 'args', 'decorators', 'play functions', 'testing'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/writing-stories" });
 * const { result }   = await invoke("search-docs", { query: "play functions testing" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown storybook-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Storybook doc path:
 *  - Strips absolute URLs to path-only
 *  - Removes .md / index.md suffixes
 *  - Ensures leading /
 */
export function normalizeStorybookDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching Storybook index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Storybook docs index not found (404). The site may have changed its structure."
        : `Failed to fetch Storybook llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeStorybookDocPath(path);
  ...
  throw new Error(`Storybook page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading Storybook docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Storybook full docs archive not found (404). The site may have changed its structure."
        : `Could not load Storybook full docs: ${res.status}`,
    );
  }

 ...
 * tools/zod-docs.js
 *
 * Fetch Zod schema validation library documentation — no API key required.
 * Zod exposes docs as Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index   → lists all available Zod doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Zod docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Zod documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Zod doc page as clean Markdown. " +
      "Examples: '/introduction', '/primitives', '/objects', '/error-handling'.",
    parameters: {
      path: "string — doc path, e.g. '/primitives'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Zod documentation for a keyword or topic. " +
      "Good for questions like 'string validation', 'error handling', 'custom schemas'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/primitives" });
 * const { result }   = await invoke("search-docs", { query: "error handling" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown zod-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Zod doc path:
 *  - Strips absolute URLs to path-only
 *  - Removes .md / index.md suffixes
 *  - Ensures leading /
 *  - Strips query params and hash
 */
export function normalizeZodDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching Zod index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Zod docs index not found (404). The site may have changed its structure."
        : `Failed to fetch Zod llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeZodDocPath(path);
  ...
  throw new Error(`Zod page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading Zod docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Zod docs archive not found (404). The site may have changed its structure."
        : `Could not load Zod docs: ${res.status}`,
    );
  }

 ...
 * tools/solid-docs.js
 *
 * Fetch SolidJS documentation — no API key required.
 * SolidJS exposes docs as clean Markdown via llms.txt endpoint.
 *
 * Tools:
 *   - get-index    → lists all available SolidJS doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all SolidJS docs
 */

const BASE = "https://docs.solidjs.com";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all SolidJS documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific SolidJS doc page as clean Markdown. " +
      "Use a path like '/concepts/signals' or '/reference/component-apis/create-signal'.",
    parameters: {
      path: "string — doc path, e.g. '/guides/getting-started'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all SolidJS documentation for a keyword or topic. " +
      "Good for questions like 'how do signals work' or 'solid routing'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/concepts/signals" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown solid-docs tool: "${toolName}"`);
  }
}

export function normalizeSolidDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch SolidJS llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeSolidDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`SolidJS page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  // SolidJS does not have llms-full.txt — use llms.txt instead
  const res = await fetch(`${BASE}/llms.txt`);
  if (!res.ok) throw new Error(`Failed to fetch SolidJS docs: ${res.status}`);

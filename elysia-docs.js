/**
 * tools/elysia-docs.js
 *
 * Fetch Elysia web framework documentation — no API key required.
 * Elysia exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Elysia doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Elysia docs
 */

const BASE = "https://elysiajs.com";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Elysia documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Elysia doc page as clean Markdown. " +
      "Use a path like '/essential/route' or '/patterns/cookie'.",
    parameters: {
      path: "string — doc path, e.g. '/essential/handler'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Elysia documentation for a keyword or topic. " +
      "Good for questions like 'how does validation work' or 'elysia lifecycle'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/essential/route" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown elysia-docs tool: "${toolName}"`);
  }
}

export function normalizeElysiaDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Elysia llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeElysiaDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Elysia page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Elysia full docs: ${res.status}`);

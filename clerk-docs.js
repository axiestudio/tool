 ...
 * tools/clerk-docs.js
 *
 * Fetch Clerk authentication & user management documentation — no API key required.
 * Clerk exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Clerk doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Clerk docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Clerk documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Clerk doc page as clean Markdown. " +
      "Use a path like '/docs/quickstarts/nextjs' or '/docs/authentication/overview'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/organizations/overview'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Clerk documentation for a keyword or topic. " +
      "Good for questions like 'how does session management work' or 'clerk webhooks'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/quickstarts/nextjs" });
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown clerk-docs tool: "${toolName}"`);
  }
}

export function normalizeClerkDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Clerk llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeClerkDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Clerk page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) {
    ...
    if (!res.ok) throw new Error(`Failed to fetch Clerk docs: ${res.status}`);
  }

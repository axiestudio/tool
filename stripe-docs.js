/**
 * tools/stripe-docs.js
 *
 * Fetch Stripe payment platform documentation — no API key required.
 * Stripe exposes a comprehensive llms.txt catalog with 1300+ pages
 * covering Payments, Billing, Connect, Terminal, Identity, Tax, and more.
 *
 * Tools:
 *   - get-index   → lists all available Stripe doc pages
 *   - get-page    → fetches a specific doc page as Markdown
 *   - search-docs → searches across all Stripe docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Stripe documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Stripe doc page as clean Markdown. " +
      "Examples: '/payments/accept-a-payment', '/billing/subscriptions/overview', '/api/charges'.",
    parameters: {
      path: "string — doc path, e.g. '/payments/accept-a-payment'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Stripe documentation for a keyword or topic. " +
      "Good for questions like 'subscriptions', 'webhooks', 'connect oauth', 'payment intents'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/payments/accept-a-payment" });
 * const { result }   = await invoke("search-docs", { query: "subscriptions billing" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown stripe-docs tool: "${toolName}"`);
  }
}

/**
 * Normalize a Stripe doc path:
 *  - Strips absolute URLs to path-only
 *  - Removes .md extension
 *  - Ensures leading /
 *  - Strips query params and hash
 */
export function normalizeStripeDocPath(path) {
  if (!path) return "";
...
async function getIndex() {
  ...
  try {
    ...
    throw new Error(`Network error fetching Stripe index: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Stripe docs index not found (404). The site may have changed its structure."
        : `Failed to fetch Stripe llms.txt: ${res.status}`,
    );
  }
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeStripeDocPath(path);
  ...
  throw new Error(`Stripe page not found: ${path}. Check the path or use get-index to discover available pages.`);
}
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  try {
    ...
    throw new Error(`Network error loading Stripe docs: ${err.message}`);
  }
  if (!res.ok) {
    throw new Error(
      res.status === 404
        ? "Stripe docs index not found (404). The site may have changed its structure."
        : `Could not load Stripe docs: ${res.status}`,
    );
  }

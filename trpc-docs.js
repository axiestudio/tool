 ...
 * tools/trpc-docs.js
 *
 * Fetch tRPC documentation — no API key required.
 * tRPC exposes all docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available tRPC doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all tRPC docs
 */

const BASE = "https://trpc.io";

export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all tRPC documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific tRPC doc page as clean Markdown. " +
      "Use a path like '/docs/getting-started' or '/docs/client/react'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/client/react'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all tRPC documentation for a keyword or topic. " +
      "Good for questions like 'how do subscriptions work' or 'tRPC react query'.",
    parameters: {
...
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown trpc-docs tool: "${toolName}"`);
  }
}

export function normalizeTrpcDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch tRPC llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeTrpcDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`tRPC page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch tRPC full docs: ${res.status}`);

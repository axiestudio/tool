/**
 * tools/supabase-docs.js
 *
 * Fetch Supabase documentation — no API key required.
 * Supabase exposes docs as clean Markdown via llms.txt endpoints.
 *
 * Tools:
 *   - get-index    → lists all available Supabase doc pages
 *   - get-page     → fetches a specific doc page as Markdown
 *   - search-docs  → searches across all Supabase docs
 */
...
export const tools = [
  {
    name: "get-index",
    description:
      "Returns a full index of all Supabase documentation pages. " +
  ...
  },
  {
    name: "get-page",
    description:
      "Fetches a specific Supabase doc page as clean Markdown. " +
      "Use a path like '/docs/guides/getting-started' or '/docs/guides/auth'.",
    parameters: {
      path: "string — doc path, e.g. '/docs/guides/database'",
    },
  },
  {
    name: "search-docs",
    description:
      "Searches all Supabase documentation for a keyword or topic. " +
      "Good for questions like 'row level security' or 'edge functions'.",
    parameters: {
 ...
 * const { markdown } = await invoke("get-page", { path: "/docs/guides/auth" });
 * const { result }   = await invoke("search-docs", { query: "row level security" });
 */
export async function invoke(toolName, args = {}) {
  switch (toolName) {
    ...
    default:
      throw new Error(`Unknown supabase-docs tool: "${toolName}"`);
  }
}

export function normalizeSupabaseDocPath(raw) {
  let normalized = raw;
...
async function getIndex() {
  ...
  if (!res.ok) throw new Error(`Failed to fetch Supabase llms.txt: ${res.status}`);
...
async function getPage({ path }) {
  ...
  const cleanPath = normalizeSupabaseDocPath(path);
  ...
  if (!res.ok) {
    throw new Error(`Supabase page not found: ${path}`);
  }
...
async function searchDocs({ query, maxChars = 8000 }) {
  ...
  // Try the main guides doc first (most comprehensive)
  const guidesRes = await fetch(`${BASE}/llms/guides.txt`);
  if (guidesRes.ok) {
    text = await guidesRes.text();
  } else {
    // Fall back to hub llms.txt
    const fallbackRes = await fetch(`${BASE}/llms.txt`);
    if (!fallbackRes.ok) throw new Error(`Failed to fetch Supabase docs: ${fallbackRes.status}`);
